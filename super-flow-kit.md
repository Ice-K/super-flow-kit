...
3. 每个问题都必须保留用户自定义入口，例如“其他，我自己说明”。
4. 不应强迫用户只能从预设选项中选择。
...
1. 读取当前模块状态。
2. 将 `requirement.status` 更新为 `in_progress`。
3. 读取用户输入的初始需求。
4. 通过苏格拉底式提问澄清需求；澄清过程应采用单题推进，每次只提出一个关键问题。
5. 每个问题可根据场景设计为单选、多选或自由输入，并必须保留用户自定义入口。
6. 所有关键问题回答完成后，先总结用户已确认的信息，并请求用户确认是否生成草稿。
7. 用户确认总结后，生成结构化需求分析文档草稿。
8. 保存 Markdown 文件到 `docs/super-flow-kit/{moduleId}/`。
9. 更新 `state.json` 中 `artifacts.requirement`，此时默认保持 `status: in_progress` 和 `quality: draft`。
10. 记录历史事件。
11. 请求用户确认该文档是否可作为后续阶段依据。
12. 用户明确确认后，才将 `requirement.status` 标记为 `done`，质量标记为 `confirmed`。
...
- 已存在已确认需求文档：提示用户选择继续编辑、生成新版本或取消。
...
3. 每个问题都应允许用户自定义回答。
4. 如果提供选项，每个问题都必须包含“其他，我自己说明”或等价入口。
