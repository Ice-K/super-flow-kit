...
当前实现仍以需求、设计和开发计划产出物闭环为主：需求分析已具备必备章节与确认前模板占位符门禁；UI 设计 artifact 流程已补齐；系统设计闭环已具备脚本层兜底，并在 v0.3.1 明确强化 API 设计和数据库设计；v0.4 开始支持开发文档/开发计划产出物，v0.4.1 增加源码实现二次确认门禁。测试、部署阶段继续保留一致的入口和状态字段：
...
以下命令中，UI 设计已作为窄范围切片实现 artifact 闭环，系统设计已增强为带 API/数据库设计门禁的闭环，开发阶段已支持基础开发文档/开发计划闭环，测试和部署仍为预留入口：
...
本章定义 `super-flow-kit` 的阶段模型。v0.1 完整实现需求分析阶段；当前实现已补齐 UI 设计 artifact 闭环、带脚本层兜底的系统设计闭环、API/数据库设计补强和 v0.4 开发文档闭环。状态模型从一开始支持完整六阶段，避免后续版本迁移成本过高。
...
1. 看板展示后续阶段为什么尚未开始。
2. 阻止系统设计和开发阶段在硬依赖缺失时写入产出物。
3. 在软依赖缺失时提示假设和复核风险。
4. 为后续测试、部署阶段实现完整依赖补全策略预留结构。
