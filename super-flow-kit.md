...
v0.1 MVP 只完整实现“需求分析”阶段；当前实现已在后续切片中补齐 `/sfk-ui` UI 设计 artifact 闭环和 `/sfk-design` 系统设计闭环。系统设计写入层会强制需求硬依赖、必备章节和确认前模板占位符清理。开发、测试、部署等阶段仍先保留状态字段、命令占位和文档模板预留，后续版本逐步实现。
...
当前实现仍以需求分析为最完整闭环，并已补齐 UI 设计 artifact 流程和带脚本层兜底的系统设计闭环；后续开发、测试、部署阶段继续保留一致的入口和状态字段：
