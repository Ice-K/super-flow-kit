...
当前实现仍以需求分析为最完整闭环，并已具备需求文档必备章节与确认前模板占位符门禁；同时已补齐 UI 设计 artifact 流程和带脚本层兜底的系统设计闭环；后续开发、测试、部署阶段继续保留一致的入口和状态字段：
...
以下命令中，UI 设计已作为窄范围切片实现 artifact 闭环，系统设计已增强为带脚本层兜底的闭环，开发、测试、部署仍为预留入口：

| 命令          | 阶段     | 当前响应                                                             |
| ------------- | -------- | -------------------------------------------------------------------- |
| `/sfk-ui`     | UI 设计  | 已支持基于需求和 UI 上下文生成或更新 UI 设计文档。                   |
| `/sfk-design` | 系统设计 | 已支持基于已确认需求，并可选结合 UI/代码上下文生成或更新系统设计文档。 |
...
本章定义 `super-flow-kit` 的阶段模型。v0.1 完整实现需求分析阶段；当前实现已补齐 UI 设计 artifact 闭环和带脚本层兜底的系统设计闭环。状态模型从一开始支持完整六阶段，避免后续版本迁移成本过高。
...
| 3        | `system_design` | 系统设计 | 定义架构、技术选型、接口、数据模型和关键约束。 | `系统设计.md` | 闭环实现     |
