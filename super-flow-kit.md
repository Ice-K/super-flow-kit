...
| 系统设计 | `system_design` | 明确技术架构、数据库设计、API / 接口设计和关键技术选型。 | 系统设计文档       |
| 软件开发 | `development`   | 根据已确认需求和设计制定开发计划、任务拆解和验证策略。 | 开发文档       |
| 功能测试 | `testing`       | 设计并执行测试，验证功能是否满足验收标准。       | 测试用例、测试报告 |
| 部署上线 | `deployment`    | 规划部署、监控、回滚和上线操作。                 | 部署文档、运维说明 |

v0.1 MVP 只完整实现“需求分析”阶段；当前需求分析已补齐脚本层质量门禁，写入层会检查必备章节并在确认前阻塞模板占位符。当前实现也已在后续切片中补齐 `/sfk-ui` UI 设计 artifact 闭环和 `/sfk-design` 系统设计闭环。系统设计写入层会强制需求硬依赖、必备章节和确认前模板占位符清理。开发、测试、部署等阶段仍先保留状态字段、命令占位和文档模板预留，后续版本逐步实现。
...
3. **检查**：每次新增、编辑或合并产出物文档后，必须执行通用文档检查，发现可确定错误时应自行修复，例如标题编号重复/跳号、文档信息质量状态与 state 不一致、确认后的变更记录负责人缺失等。
4. **登记**：文档路径、版本、质量状态、更新时间、负责人等信息写入 `.sfk/modules/{moduleId}/state.json`。
5. **确认**：用户确认后，产出物质量状态从 `draft` 变为 `confirmed`，阶段状态可进入 `done`。
6. **复用**：后续阶段依赖前序阶段产出物，而不是依赖聊天上下文。
7. **修改**：用户可以后续编辑产出物；插件应能在状态看板中提示产出物位置和当前状态。
...
6. 每次保存草稿或确认产出物后，应执行通用产出物文档检查，并自行修复可确定的格式和状态同步问题。
7. 用户确认后，需求分析阶段应标记为 `done`，产出物质量应标记为 `confirmed`。
...
当前实现仍以需求、设计、开发计划和功能测试产出物闭环为主：需求分析已具备必备章节与确认前模板占位符门禁；UI 设计 artifact 流程已补齐；系统设计闭环已具备脚本层兜底，并在 v0.3.1 明确强化 API 设计和数据库设计；v0.4 开始支持开发文档/开发计划产出物，v0.4.1 增加源码实现二次确认门禁；v0.5 开始支持测试文档产出物并增加脚本层质量门禁。部署阶段继续保留一致的入口和状态字段：

| 命令          | 阶段     | 当前行为                                                                 |
| ------------- | -------- | ------------------------------------------------------------------------ |
| `/sfk-ui`     | UI 设计  | 已支持进入 UI 设计阶段，生成或更新 UI 设计草稿，并经用户确认后标记完成。 |
| `/sfk-design` | 系统设计 | 已支持在需求确认后进入系统设计阶段，生成或更新系统设计草稿并确认。       |
| `/sfk-dev`    | 软件开发 | v0.4 支持进入开发阶段，生成或更新开发文档/开发计划；v0.4.1 增加源码实现二次确认门禁，不自动修改业务代码。 |
| `/sfk-test`   | 功能测试 | v0.5 支持进入功能测试阶段，基于已确认需求生成或更新测试文档；系统设计和开发文档作为软依赖提示风险。 |
| `/sfk-deploy` | 部署上线 | v0.6 支持进入部署上线阶段，生成或更新部署文档；部署前检查所有模块测试产出物是否已确认且可用。 |

未实现阶段不应假装已经完成能力。若用户调用尚未完整实现的命令，应明确说明当前版本限制，并给出可执行的替代建议。
...
9. 保存草稿时，脚本层必须检查需求文档必备章节，并允许草稿阶段保留模板占位符但输出 `qualityCheck：warnings`。
10. 用户明确确认后，脚本层必须再次检查需求文档；若仍有模板占位符或缺少必备章节，不得标记为 `done/confirmed`。
...
以下命令中，UI 设计已作为窄范围切片实现 artifact 闭环，系统设计已增强为带 API/数据库设计门禁的闭环，开发阶段已支持基础开发文档/开发计划闭环，测试阶段已支持测试文档闭环，部署阶段已支持部署文档闭环：

| 命令          | 阶段     | 当前响应                                                             |
| ------------- | -------- | -------------------------------------------------------------------- |
| `/sfk-ui`     | UI 设计  | 已支持基于需求和 UI 上下文生成或更新 UI 设计文档。                   |
| `/sfk-design` | 系统设计 | 已支持基于已确认需求，并可选结合 UI/代码上下文生成或更新系统设计文档；系统设计必须包含 API 和数据库设计。 |
| `/sfk-dev`    | 软件开发 | v0.4 已支持基于已确认需求和系统设计生成或更新开发文档/开发计划；v0.4.1 要求源码实现前二次确认。 |
| `/sfk-test`   | 功能测试 | v0.5 已支持基于已确认需求以及可选系统设计/开发文档生成或更新测试文档；测试文档确认前必须通过必备章节和占位符门禁。 |
| `/sfk-deploy` | 部署上线 | v0.6 已支持生成或更新部署文档；testing 是部署软依赖，但部署前必须检查所有模块测试产出物状态并提示风险。 |

部署命令应遵循两个原则：

1. 不夸大能力：部署文档 confirmed 不代表真实部署已经执行。
...
本章定义 `super-flow-kit` 的阶段模型。v0.1 完整实现需求分析阶段；当前实现已补齐 UI 设计 artifact 闭环、带脚本层兜底的系统设计闭环、API/数据库设计补强、v0.4 开发文档闭环、v0.5 测试文档闭环和 v0.6 部署文档闭环。状态模型从一开始支持完整六阶段，避免后续版本迁移成本过高。
...
| 阶段顺序 | 阶段 ID         | 展示名称 | 核心职责                                       | 产出物后缀    | 当前状态     |
| -------- | --------------- | -------- | ---------------------------------------------- | ------------- | ------------ |
| 1        | `requirement`   | 需求分析 | 澄清目标、范围、用户故事和验收标准。           | `需求分析.md` | 完整实现     |
| 2        | `ui_design`     | UI 设计  | 设计页面结构、交互流程、视觉规范和组件清单。   | `UI设计.md`   | 窄切片实现   |
| 3        | `system_design` | 系统设计 | 定义架构、技术选型、API 设计、数据库设计和关键约束。 | `系统设计.md` | 闭环实现     |
| 4        | `development`   | 软件开发 | 制定开发计划、任务拆解、关键文件改动和验证策略。      | `开发文档.md` | v0.4 基础实现 |
| 5        | `testing`       | 功能测试 | 设计测试策略、测试用例并记录测试结果。         | `测试文档.md` | v0.5 基础实现 |
| 6        | `deployment`    | 部署上线 | 设计部署流程、监控告警、回滚方案和上线说明。   | `部署文档.md` | v0.6 基础实现 |
...
| `deployment`    | 部署上线 | 无                             | `testing`、`system_design`     |

当前实现中，依赖检测用于：

1. 看板展示后续阶段为什么尚未开始。
2. 阻止系统设计、开发和测试阶段在硬依赖缺失时写入产出物。
3. 在软依赖缺失时提示假设和复核风险。
4. 部署阶段通过 `deployment readiness` 检查所有模块测试产出物状态并提示风险，但不把当前模块 testing 作为硬阻塞依赖。
  ...
  "artifacts": {
    ...
    "development": {
      ...
      "implementationApproval": {
        ...
        "approvedAt": null,
        "approvedBy": null,
        "approvedForFile": null,
        "approvedForConfirmedAt": null,
        "summary": null
      }
    },
    "testing": {
...
| `artifacts.development.implementationApproval` | state | 源码实现二次确认状态；仅表示是否允许开始按开发文档修改源码，不表示实现完成。 |
...
v0.2 已实现，模板文件为 `templates/ui-design.md`。
...
v0.3 已实现，v0.3.1 已补强 API 设计和数据库设计，模板文件为 `templates/system-design.md`。
...
v0.4 基础实现，模板文件为 `templates/development.md`。该文档用于制定开发计划和任务拆解，不代表业务代码已经实现。
...
`/sfk-reset` 属于高风险操作。v0.9 实现状态级 reset：先通过 `reset impact` 只读展示影响范围，再通过 `reset apply --confirm 确认重置` 在严格二次确认后重置 `.sfk` 工作流状态。v0.9 默认保留 `docs/super-flow-kit/` 下的产出物文档、导出包和业务源码，只解除 `.sfk` 当前状态引用并追加 history。任何重置、删除或覆盖状态的操作都需要二次确认。
...
- UI 设计文档模板
- UI 阶段软依赖检查
- 通用 `artifact draft/confirm/current` 状态机复用

验收标准：用户能从需求分析继续生成 UI 设计产出物。

### 14.3 v0.3 系统设计闭环

目标：基于已确认需求进入系统设计阶段，生成可确认的系统设计产出物。
...
验收标准：用户能从已确认需求继续生成系统设计产出物，并在看板中看到确认状态。

### 14.4 v0.3.1 API / 数据库设计补强
...
- 数据库设计覆盖概念模型、表结构、字段、关系、索引、唯一约束、生命周期和迁移策略。
- API / 接口设计覆盖接口清单、请求/响应、错误码、鉴权、分页、排序、过滤、幂等、限流和外部集成。
- 脚本层质量门禁将数据库设计和 API / 接口设计作为系统设计必备内容。

验收标准：系统设计草稿缺少数据库设计或 API / 接口设计时不能写入阶段状态；确认前不得残留这些章节中的模板占位符。

### 14.5 v0.4 /sfk-dev 基础开发阶段

目标：基于已确认需求和系统设计进入开发阶段，生成开发文档/开发计划产出物。

建议交付：

- `/sfk-dev` 基础开发阶段流程
- 开发文档模板
- 开发阶段硬依赖检查：需求分析和系统设计必须已确认
- UI 设计作为软依赖，可带假设继续
- 开发文档必备章节和确认前模板占位符门禁
- 开发阶段下游影响分析，提示测试和部署复核风险

验收标准：用户能从已确认需求和系统设计继续生成一份已确认的开发文档；v0.4 不自动修改业务代码，也不把开发文档确认等同于代码实现完成。

### 14.6 v0.4.1 源码实现二次确认门禁

目标：区分“开发文档已确认”和“已授权修改源码”。

建议交付：

- `scripts/sfk.py implementation current development` 查看源码实现授权状态。
- `scripts/sfk.py implementation approve development --summary ...` 记录实现授权。
- `/sfk-dev` 在开发文档确认后新增 Gate C 二次确认。
- 开发文档重新进入 draft 时，自动重置实现授权。
- 看板提示实现授权状态，避免误把 `development.done/confirmed` 理解为代码已实现。

验收标准：开发文档 confirmed 后，未通过实现授权前不得修改业务源码；授权状态可审计、可检测是否过期。
...
| 命令                        | 当前状态    | 说明                                 |
| --------------------------- | ----------- | ------------------------------------ |
| `/sfk`                      | 已实现      | 唤醒插件并显示当前状态和下一步建议。 |
| `/sfk-help`                 | 已实现      | 只读查询插件指令清单和指令说明。     |
| `/sfk-init`                 | 已实现      | 初始化项目工作流状态。               |
| `/sfk-status`               | 已实现      | 查看项目和当前模块看板。             |
| `/sfk-module create <名称>` | 已实现      | 创建模块。                           |
| `/sfk-module list`          | 已实现      | 列出模块。                           |
| `/sfk-module switch <模块>` | 已实现      | 切换当前模块。                       |
| `/sfk-module status`        | 已实现      | 查看当前模块状态。                   |
| `/sfk-req`                  | 已实现      | 执行需求分析流程。                   |
| `/sfk-config`               | 已实现      | 查看或修改项目配置。                 |
| `/sfk-ui`                   | v0.2 已实现 | UI 设计阶段。                        |
| `/sfk-design`               | v0.3 已实现 | 系统设计阶段，含 API/数据库设计补强。 |
| `/sfk-dev`                  | v0.4.1 已实现 | 软件开发计划阶段，含源码实现二次确认门禁。 |
| `/sfk-test`                 | v0.5 已实现 | 功能测试阶段，生成测试文档并执行质量门禁。 |
| `/sfk-deploy`               | v0.6 已实现 | 部署上线阶段，生成部署文档并检查全模块测试准入。 |
| `/sfk-export`               | v0.8 已实现 | 导出单模块或全项目交付包，支持可选 zip。 |
| `/sfk-reset`                | v0.9 已实现 | 状态级重置项目或当前模块工作流状态，先展示影响范围并必须严格二次确认，默认保留产出物文档和业务源码。 |
...
└── docs/
    └── super-flow-kit/
        ...
        │   ├── 20260610100000-user-auth-UI设计.md
        │   ├── 20260611100000-user-auth-系统设计.md
        │   └── 20260611110000-user-auth-开发文档.md
        └── payment-module/
