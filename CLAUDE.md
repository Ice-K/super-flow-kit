...
- `/sfk-export` 是横向导出工具，不新增 `artifacts.export`，不改变 `currentPhase`，不写 `.sfk` 状态；交付包只复制模块文档目录下的阶段 Markdown 产出物，并生成 manifest、索引、风险清单和检查清单。
