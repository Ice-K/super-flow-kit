...
- `/sfk-reset` 是高风险状态重置工具，必须先通过 `scripts/sfk.py reset impact` 展示影响范围，再由用户单独输入完全一致的 `确认重置` 后才能执行 `reset apply`；v0.9 只重置 `.sfk` 工作流状态，默认不删除 `docs/super-flow-kit/` 产出物文档、导出包或业务源码。
